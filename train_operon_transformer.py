...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the unsupervised MSA Transformer")
...
def main():
    ...
    model = operon_transformer(depth=args.depth,
                               max_num_genes=args.max_num_genes,
    ...
    for epoch in range(args.epochs):
        ...
        if epoch_val_losses[-1] < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       f'{args.save_dir}/operon_transformer_stable_{args.img_set}_{epoch}.pt')

        torch.save(model.state_dict(),
                   f'{args.save_dir}/operon_transformer_stable_last.pt')

        with open(f"{args.save_dir}/{args.img_set}.txt", "w") as outfile:
